// Compute shader for frustum and occlusion culling of meshes.
// See meshlet.glsl for task shader that does the same for meshlets.

#if defined(COMPUTE_GPU_CULLING)

layout(set = MATERIAL_SET, binding = 1) writeonly buffer VisibleMeshInstances {
	// When a mesh passes frustum and occlusion culling testing, a draw
	// command for it is written here.
...
// See https://jcgt.org/published/0002/02/05/.
//
// Returns an axis-aligned bounding box in screen space of the input world-space bounding sphere.
//
// C is the center of the bounding sphere and r is its radius.
// P00 and P11 are the first 2 diagonal values of the view-projection matrix.
bool project_sphere(vec3 C, float r, float znear, float P00, float P11, out vec4 aabb) {
	if (-C.z - r < znear) {
		// The bounding sphere is behind the near plane.
		return false;
	}

	// Compute minimum and maximum points on the x axis (in the xz plane) in world space.
	vec2 cx = vec2(C.x, -C.z);
	vec2 vx = vec2(sqrt(dot(cx, cx) - r * r), r);
	vec2 minx = mat2(vx.x, vx.y, -vx.y, vx.x) * cx;
	vec2 maxx = mat2(vx.x, -vx.y, vx.y, vx.x) * cx;

	// Compute minimum and maximum points on the y axis (in the yz plane) in world space.
	vec2 cy = -C.yz;
	vec2 vy = vec2(sqrt(dot(cy, cy) - r * r), r);
	vec2 miny = mat2(vy.x, vy.y, -vy.y, vy.x) * cy;
	vec2 maxy = mat2(vy.x, -vy.y, vy.y, vy.x) * cy;

	// Build an axis-aligned bounding box for the sphere using these points, transforming the aabb
	// to perspective-projected screen space along the way.
	aabb = vec4(minx.x / minx.y * P00, miny.x / miny.y * P11, maxx.x / maxx.y * P00, maxy.x / maxy.y * P11);

	// Transform aabb from screen space (where coordinates are in the range [-1, 1])
	// to UV space (where coordinates are in the range [0, 1]).
	//
	// y coordinates are multiplied by a negative number because screen space has a
	// bottom-left origin, while UV space has a top-left origin.
...
void main() {
	// TODO: there's no mention of mesh instances in the book.
	...
	if (mesh_instance_index < count) {
		...
		// A MeshDraw contains the mesh's textures, colors, and meshlet info. See mesh.h.
		...
		// The meshes bounding sphere is used to do frustum culling.
		...
		// Inflate bounding sphere. Scale is obtained from the model matrix?
    	float radius = bounding_sphere.w * scale * 1.1;

		// Check bounding sphere against the 6 planes of the camera frustum.
    	bool frustum_visible = true;
	    for (uint i = 0; i < 6; ++i) {
	        frustum_visible = frustum_visible && (dot(frustum_planes[i], view_bounding_center) > -radius);
	    }

		// TODO: what does frustum_cull_meshes do?
	    frustum_visible = frustum_visible || (frustum_cull_meshes == 0);

		// If the mesh passes frustum culling, do occlusion culling.
	    bool occlusion_visible = true;
	    if (frustum_visible) {
	    	vec4 aabb;
			// project_sphere returns false when the bounding sphere is completely
			// behind the near plane.
	    	if (project_sphere(view_bounding_center.xyz, radius, z_near, projection_00 , projection_11, aabb)) {
				// aabb (produced by project_sphere()) is a UV space projection of the 
				// axis-aligned bounding box of the mesh's bounding sphere.

				// Determine the mip level of the depth pyramid that should be used.
    			ivec2 depth_pyramid_size = textureSize(global_textures[nonuniformEXT(depth_pyramid_texture_index)], 0);
				// aabb.z - aabb.x yields a number between 0 and 1, so width is a fraction
				// of the width of the top level texture of the depth pyramid. Same goes for height.
				...
				// Look up the current depth at this UV coordinate.
				...
				// Compute the closest depth of the bounding sphere.
	    ...
	    if (frustum_visible && occlusion_visible) {
	    	// Mesh passed frustum and occlusion testing. Record draw command for it.
			// The draw_commands buffer records up to total_count opaque draws followed
			// by transparent draws.

			if ((flags & (DrawFlags_AlphaMask | DrawFlags_Transparent)) == 0) {
				// Opaque draws. Record it right after the last opaque draw.
				...
				// Transparent draw. Note that its command is not written in the next
				// available slot, but after total_count commands in the same buffer.
		...
		// Commands recorded in draw_commands will be used to draw meshlets of visible meshes.
	}
}
