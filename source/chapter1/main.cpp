...
int main(int argc, char** argv) {
    ...
    // Initialize resource manager. Resources include textures, buffers, and samplers.
    ...
    // Allocate and create texture resources from images found in the scene.
    ...
    // Allocate and initialize samplers.
    ...
    // Allocate buffers and read data into them from buffers found in the scene.
    ...
    // Mesh draws are objects that contain all the data for a draw call.
...

    {
        ...
        // Vertex tangent.
        pipeline_creation.vertex_input.add_vertex_attribute({ 1, 1, 0, VertexComponentFormat::Float4 });
        pipeline_creation.vertex_input.add_vertex_stream({ 1, 16, VertexInputRate::PerVertex});
        // Vertex normal.
        pipeline_creation.vertex_input.add_vertex_attribute({ 2, 2, 0, VertexComponentFormat::Float3 });
        pipeline_creation.vertex_input.add_vertex_stream({ 2, 12, VertexInputRate::PerVertex });
        // Vertex texture coordinate.
        pipeline_creation.vertex_input.add_vertex_attribute({ 3, 3, 0, VertexComponentFormat::Float2 });
        ...
        // Set shaders.
        pipeline_creation.shaders.set_name("Cube")
        .add_stage(vs_code, (uint32_t) strlen(vs_code), VK_SHADER_STAGE_VERTEX_BIT)
        .add_stage(fs_code, (uint32_t) strlen(fs_code), VK_SHADER_STAGE_FRAGMENT_BIT);

        // Descriptor set layout. A descriptor is a way for shaders to freely access resources like buffers and images.
        ...
        for (u32 mesh_index = 0; mesh_index < scene.meshes_count; ++mesh_index) {
            ...
            for (u32 primitive_index = 0; primitive_index < mesh.primitives_count; ++primitive_index) {
                ...
                // Populate MeshDraw with position buffer.
                ...
                // Populate MeshDraw with tangent buffer.
                ...
                // Populate MeshDraw with normal buffer.
                ...
                // Populate MeshDraw with texture coordinate buffer.
                ...
                // Populate MeshDraw with index buffer.
    ...
    // Destroy and shut down everything.
    for (u32 mesh_index = 0; mesh_index < mesh_draws.size; ++mesh_index) {
        MeshDraw &mesh_draw = mesh_draws[mesh_index];
